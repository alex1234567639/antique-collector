...
onMounted(async () => {
  ...
  try {
    ...
    message.value = err instanceof Error ? err.message : "載入失敗";
  } finally {
    loading.value = false;
  }
});
    ...
    <div class="dialog">
      ...
      <form class="form">
        ...
        <p
          ...
          class="message"
          :class="{
            error: message.includes('失敗') || message.includes('找不到'),
          }"
        >
          {{ message }}
        </p>
